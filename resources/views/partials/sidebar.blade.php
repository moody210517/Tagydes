...
<div id="sidebar" class="sidebar">
	...
	<div data-scrollbar="true" data-height="100%">
		...
		<ul class="nav">
			...
			<li class="has-sub {{ Request::is('reseller*') || Request::is('customer*') || Request::is('subscription*') || Request::is('order*') || Request::is('task*') || Request::is('news*') || Request::is('boffice*') ? 'active' : ''  }}">
				<a href="javascript:;">
					<b class="caret"></b>
				...
				<ul class="sub-menu">
					...
					@if( auth()->user()->getRole() == 5 && auth()->user()->parentUserRole() == 2 )					
					@else
					...
					<li class="has-sub {{ Request::is('reseller*') || Request::is('customer*') || Request::is('subscription*') || Request::is('order*') || Request::is('task*') || Request::is('news*')|| Request::is('boffice*') ? 'active' : ''  }}">
						...
						<ul class="sub-menu">
						<?php $user = auth()->user(); $branches = $user->getBranches(); ?>

						@foreach($branches as $branch)
						@if($user->getRole() == 5)
						<li class="{{ Request::get('branch_id') == $branch->branch_id ? 'active' : ''  }}">
							<a href="{{ route('branch_office.list', ['branch_id' =>  $branch->branch_id ]) }}">
								<i class="fas fa-users"></i>							
								<span>{{$branch->title}}</span>
							</a>
						</li>
						@else
						<li class="{{ Request::get('branch_id') == $branch->id ? 'active' : ''  }}">
							<a href="{{ route('branch_office.list', ['branch_id' =>  $branch->id ]) }}">
								<i class="fas fa-users"></i>							
								<span>{{$branch->title}}</span>
							</a>
						</li>
			...
			<li class="has-sub {{ Request::is('invoice*') || Request::is('product*') || Request::is('pricelist*') || Request::is('user*') || Request::is('activity*') || Request::is('permission*') || Request::is('role*') || Request::is('supervisor*')  || Request::is('branch*') ? 'active' : ''  }}">
				<a href="javascript:;">
					<b class="caret"></b>
