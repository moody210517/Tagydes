...
class UsersControllerTest extends FunctionalTestCase
{
    ...
    public function test_users_table_is_displayed()
    {
        ...
        $this->visit('user')
            ->seeInElement("h4.page-header", "Users")
            ->see("Add User");

        foreach ($users as $i => $user) {
            $this->seeInTable("#users-table-wrapper table", $user->username, $i + 1, 2);
            $this->seeInTable("#users-table-wrapper table", $user->present()->name, $i + 1, 3);
            $this->seeInTable("#users-table-wrapper table", $user->email, $i + 1, 4);
            $this->seeInTable("#users-table-wrapper table", $user->created_at->format('Y-m-d'), $i + 1, 5);
            $this->seeInTable("#users-table-wrapper table", $user->status, $i + 1, 6);
        }
    }
    ...
    public function test_users_search()
    {
        ...
        $this->visit('user')
            ...
            ->seeInTable('table', $user3->present()->name, 1, 3)
            ->seeInTable('table', $user2->present()->name, 2, 3);
    }
